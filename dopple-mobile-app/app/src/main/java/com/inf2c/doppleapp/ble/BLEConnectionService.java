...
public class BLEConnectionService extends Service {
    ...
    private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                gatt.requestMtu(515);
                DoppleLog.i(TAG, "Connected to GATT server. Requesting MTU 515 now.");
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                DoppleLog.i(TAG, "Gatt Connected. Attempting to start service discovery:" + mBluetoothGatt.discoverServices());
            } else {
                DoppleLog.e(TAG, "Could not set MTU " + mtu);
            }
        }
        ...
        public void onDescriptorRead(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            //check if we can read the descriptor from the start, if so, we can access the data:
            if (status == BluetoothGatt.GATT_SUCCESS) {
                broadcastUpdate(ACTION_GATT_CONNECTED);
            }
            if (status == 1) { //1 means invalid handle
                // this means it couldnt get the notify property working on the earbuds. that also means it's not running the correct firmware.
                ContextCompat.getMainExecutor(getApplicationContext()).execute(() -> {
                    Toast.makeText(getApplicationContext(), "Incorrect firmware version.", Toast.LENGTH_LONG).show();
                });
     ...
     * @return returns if it could set the notification or not.
     */
    ...
    private void startACCDataReceiver() {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                //check if this charachteristic is allowed to get written to.
                //enabled the notify option on the 4th characteristics (raw)
                if (setNotificationOnGattCharacteristics(DoppleGattAttributes.RAW_ACC_DATA)) {
                    startTimer();
                    handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
     ...
     * @param charId UUID of the characteristic
     */
    private boolean setNotificationOnGattCharacteristics(String charId) {
        if (mGattCharacteristics != null) {
            final BluetoothGattCharacteristic characteristic = characteristicSearch(charId);
            final int charaProp = characteristic.getProperties();
            if ((charaProp | BluetoothGattCharacteristic.PROPERTY_NOTIFY) > 0) {
                return BLEConnectionService.this.setCharacteristicNotification(characteristic, true);
            }
        }
    ...
    private final BroadcastReceiver gattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_GATT_CONNECTED.equals(action)) {
                ...
                //brandon: after reading the service data, check if we have write permissions on the acc charachteristic
                BluetoothGattCharacteristic characteristic = characteristicSearch(DoppleGattAttributes.PROCESSED_ACC_DATA);
                BluetoothGattDescriptor descripto1 = characteristic.getDescriptor(UUID.fromString(DoppleGattAttributes.DOPPLE_DESCRIPTOR));
                mBluetoothGatt.readDescriptor(descripto1);
